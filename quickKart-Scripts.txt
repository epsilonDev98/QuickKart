...
DROP TABLE Roles
GO
IF OBJECT_ID('usp_AddProductToCart')  IS NOT NULL
DROP PROC usp_AddProductToCart
GO
IF OBJECT_ID('ufn_FetchCartProductByEmailId')  IS NOT NULL
DROP FUNCTION ufn_FetchCartProductByEmailId
GO
IF OBJECT_ID('usp_AddProductRatings')  IS NOT NULL
DROP PROC usp_AddProductRatings
GO
...
CREATE TABLE Cart
(
	[ProductId] CHAR(4) CONSTRAINT fk_Cart_ProductId REFERENCES Products(ProductId) NOT NULL,
...
CREATE TABLE Rating
(
	[EmailId] VARCHAR(50) CONSTRAINT fk_Rating_Users_EmailId FOREIGN KEY references Users(EmailId),
	[ProductId] CHAR(4) CONSTRAINT fk_Rating_Products_ProductId FOREIGN KEY references Products(ProductId),
	...
	CONSTRAINT pk_EmailId_ProductId_Rating Primary KEY(EmailId,ProductId)
)
GO
...
CREATE FUNCTION ufn_FetchCartProductByEmailId(@EmailId Varchar(50))
RETURNS TABLE 
AS
RETURN (SELECT c.ProductId,p.ProductName,p.Price,c.Quantity,p.QuantityAvailable FROM Cart c JOIN Products p on c.ProductId=p.ProductId WHERE c.EmailId=@EmailId)
GO
...
CREATE PROCEDURE usp_AddProductRatings
(
@EmailId VARCHAR(50),
@ProductId CHAR(4),
...
BEGIN TRY 
	...
	IF NOT EXISTS(SELECT 1 from Products where ProductId=@ProductId)
		SET @RetVal = -2
	IF NOT(@ReviewRating>=0 and @ReviewRating<=5)
		SET @RetVal = -3
	INSERT INTO Rating VALUES(@EmailId,@ProductId,@ReviewRating,@ReviewComments)
	SET @RetVal = 1
	SELECT @RetVal
END TRY
BEGIN CATCH
...
CREATE PROCEDURE usp_AddProductToCart
(
	@ProductId CHAR(4),
	@EmailId VARCHAR(50)
)
	...
	BEGIN TRY		
			BEGIN						
				...
				IF(@number IS NOT NULL)
					BEGIN
						UPDATE Cart SET Quantity=Quantity+1 WHERE ProductId=@ProductId AND EmailId=@EmailId
						SET @retval = 1
					END
				ELSE
					BEGIN
						INSERT INTO Cart VALUES (@ProductId,@EmailId,1)
						SET @retval = 1
					END				
			END
		SELECT @retval 
...
INSERT INTO CardDetails(CardNumber,NameOnCard,CardType,CVVNumber,ExpiryDate,Balance) VALUES(1245674190696670,'Mary','M',828,'2019-01-04',14078.00)
...
INSERT INTO CardDetails(CardNumber,NameOnCard,CardType,CVVNumber,ExpiryDate,Balance) VALUES(1563901313189020,'Jaime Yorres','V  ',240,'2018-10-22',11605.00)
...
GO
 ...
 INSERT INTO CART VALUES ('P107','Albert@gmail.com',2)
 INSERT INTO CART VALUES ('P138','Albert@gmail.com',5)
 INSERT INTO CART VALUES ('P146','Albert@gmail.com',3)
 GO